...
### Definitions
- NFT：Non-Fungible Token (NFT), these are non-fungible tokens are used to identify something or someone in a unique way. They are defined by standard [erc-721(./erc721.md)]
...
- SBT Certificates: SBT that represents the ownerships of ID signatures corresponding to the requirements defined in `function standardRequirement()`.

- ZK ID NFT: ID NFT represents self-sovereign federated identity solving onchain verification from a source identity and serve as Zero-Knowledge Proof-of-Identity(PoID) from trusted public certificates (e.g.,national identity cards and/or ePassports, driver's license and eSIM), with ID NFT allow anonymous authentication on permissionless blockchains through multiple IT systems or even organizations to the requirements defined in `function standardRequirement()`. 
